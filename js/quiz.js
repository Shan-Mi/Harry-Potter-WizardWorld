...
const questions = [
  ...
  {
    question:
      "What is the name of the fountain located inside the Ministry of Magic?",
    choices: [
      "Fountain of Mighty Magic",
      "Magic is Might",
      "Everlasting Magic",
      "Fountain of Magical Brethen",
    ],
    correct: 3,
  },
  {
  ...
  {
    question:
      "And what platform do students need to catch it from at King's Cross?",
    choices: [
      "Platform 9 1/4",
      "Platform 9 2/4",
      "Platform 9 3/4",
  ...
  {
    question:
      "What is the name of the device that Dumbledore uses when putting out street lights?",
...
function getResults() {
  if (
    allRadios[checkedRadio].value === questions[i].choices[questions[i].correct]
  ) {
    score++;
...
function getQAs(number) {
  if (i < number) {
    ...
    for (let k = 0; k < 4; k++) {
      ...
      document
        .querySelector(`#answer${k}`)
        .setAttribute("for", questions[i].choices[k]);
      document
        .querySelector(`#q${k}`)
        .querySelector(["input"])
        .setAttribute("value", questions[i].choices[k]);
      document
        .querySelector(`#q${k}`)
        .querySelector(["input"])
        .setAttribute("id", questions[i].choices[k]);
    }
  } else {
